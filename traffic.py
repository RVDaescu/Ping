...
def main(ip, count = 100, inter = 0.01, size = 32):
    """
...
#    print 'Sending ICMP requests to %s' %ip
    send_pk = snd(host = ip, size = size, iface = 'wlo1', count = count, inter = inter)
    sleep(1)
    send_pk.start()
    ...
    ip_dict['sent'] = count
    ip_dict['reach'] = float(format(len(pkt_list)/float(count)*100, '.2f'))
